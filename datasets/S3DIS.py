...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        self.ignored_labels = np.array([255])

        # Dataset folder
        self.path = '../datasets/S3DIS_OTOC_SP'
        ...
        # self.cloud_names = ['Area_1', 'Area_5']
        # self.all_splits = [0, 1]
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            input_voxels = self.input_voxels[cloud_ind][input_inds]
            ...
            if batch_n > int(self.batch_limit) * 0.8:
                break
        ...
        voxels = np.concatenate(v_list, axis=0)
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            ##########################################################################
            ##############          save superpoint pointcloud            ############ usually False
            ##########################################################################
            ...
            if False:
                ...
                partition_points = np.empty((0, 3), dtype=np.float32)
                partition_colors = np.empty((0, 3), dtype=np.uint8)
                partition_supervoxels = np.empty((0, 1), dtype=np.int32)
                ...
                for i, room_folder in enumerate(room_folders):
                    ...
                    # for super points
                    partition = o3d.io.read_point_cloud(
                        room_folder.replace('S3DIS_OTOC_SP', 'S3DIS/clouds') + '_partition.ply')
                    color_code = numpy.asarray(partition.colors)[:, 0] * 10000 + \
                                 numpy.asarray(partition.colors)[:, 1] * 100 + \
                                 numpy.asarray(partition.colors)[:, 2] * 1
                    unique_colors = numpy.unique(color_code)
                    for index in range(len(unique_colors)):
                        color_code[color_code == unique_colors[index]] = index + last_index

                    last_index = len(unique_colors) + last_index

                    partition_points = np.vstack((partition_points, numpy.asarray(partition.points).astype(np.float32)))
                    partition_colors = np.vstack((partition_colors, (numpy.asarray(partition.colors) * 255.0).astype(np.uint8)))
                    partition_supervoxels = np.vstack((partition_supervoxels, color_code[:, None]))

                cloud_file = cloud_file.replace('original_ply', 'superpoint_ply')
                write_ply(cloud_file,
                          (partition_points, partition_colors, partition_supervoxels.astype(numpy.int32)),
                          ['x', 'y', 'z', 'red', 'green', 'blue', 'voxel'])

            ##########################################################################
            ##############       end   save superpoint pointcloud         ############
            ##########################################################################
            ...
            for i, room_folder in enumerate(room_folders):
                ...
                for object_name in listdir(join(room_folder, 'Annotations')):
                    ...
                    if object_name[-4:] == '.txt':
                        ...
                        # object_classes = np.full((object_data.shape[0], 1), 255, dtype=np.int32)
                        # object_classes[int(random.random() * object_data.shape[0])] = object_class
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                #####################################################################
                ###########            load superpoint  ############################# new added for super voxel reading
                #####################################################################
                # sub_voxel = np.zeros_like(sub_labels)

                tmp_sub_points = np.vstack((data['x'], data['y'], data['z'])).T

                sp_data = read_ply(sub_ply_file.replace('input_0.040', 'superpoint_ply'))
                tmp_sp_points = np.vstack((sp_data['x'], sp_data['y'], sp_data['z'])).T
                tmp_sp_voxels = sp_data['voxel']

                tmp_neighbor = batch_neighbors(tmp_sub_points, tmp_sp_points,
                                               [len(tmp_sub_points)], [len(tmp_sp_points)], 0.05)

                sub_voxels = tmp_sp_voxels[tmp_neighbor[:, 0]]
                ...
                if self.set == 'training':
                    ...
                    sub_labels = np.ones(sub_points.shape[0]).astype(np.int32) * 255

                    # Get chosen neighborhoods
                    search_tree = KDTree(sub_points, leaf_size=10)

                    has_label_points = points[labels != 255]
                    has_label_points_label = labels[labels != 255]
                    for i in range(has_label_points.shape[0]):
                        dist, ind = search_tree.query(has_label_points[i, None], k=1)
                        sub_labels[ind] = has_label_points_label[i]
...
class S3DISCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        ind += 1
        self.voxels = torch.from_numpy(input_list[ind])
        ind += 1
    ...
    def pin_memory(self):
        ...
        self.voxels = self.voxels.pin_memory()
...
def S3DISCollate(batch_data):
    config = batch_data[0][1]
    batch_data = [batch_data[0][0]]
    data_1 = S3DISCustomBatch(batch_data)
    data_2 = S3DISCustomBatch(copy.deepcopy(batch_data))
    data_3 = S3DISCustomBatch(copy.deepcopy(batch_data))

    R = random.random()

    if R <= 0.5: # point
        a = torch.rand(data_2.points[0].shape).sub(0.5)
        a = _l2_normalize(a)

        tmp_affine_points = data_2.points[0] + config.xi_p * a
        data_2.points[0] = tmp_affine_points.cpu().detach().numpy()
        data_2.lengths[0] = data_2.lengths[0].cpu().detach().numpy()
        r = 0.1

        for i in range(0, len(data_1.points)):
            dl = 2 * r / 2.5
            data_2.neighbors[i] = batch_neighbors(data_2.points[i], data_2.points[i],
                                                  data_2.lengths[i],
                                                  data_2.lengths[i], r)[:, :data_1.neighbors[i].shape[1]]
            if i < 4:
                data_2.points[i + 1], data_2.lengths[i + 1] = batch_grid_subsampling(data_2.points[i],
                                                                                     data_2.lengths[i],
                                                                                     sampleDl=dl)
                data_2.pools[i] = batch_neighbors(data_2.points[i + 1], data_2.points[i],
                                                  data_2.lengths[i + 1],
                                                  data_2.lengths[i], r)[:, :data_1.neighbors[i].shape[1]]
                data_2.upsamples[i] = batch_neighbors(data_2.points[i], data_2.points[i + 1],
                                                      data_2.lengths[i], data_2.lengths[i + 1],
                                                      r * 2)[:, :data_1.neighbors[i].shape[1]]

            r = r * 2
        for i in range(len(data_1.points)):

            data_2.neighbors[i] = torch.tensor(data_2.neighbors[i].astype(np.int64))
            data_2.points[i] = torch.tensor(data_2.points[i])
            data_2.lengths[i] = torch.tensor(data_2.lengths[i])
            if i < 4:
                data_2.pools[i] = torch.tensor(data_2.pools[i].astype(np.int64))
                data_2.upsamples[i] = torch.tensor(data_2.upsamples[i].astype(np.int64))


    return data_1, data_2, data_3, R  # batch, tmp_batch, affine_batch
